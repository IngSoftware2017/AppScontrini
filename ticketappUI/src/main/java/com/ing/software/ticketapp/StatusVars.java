package com.ing.software.ticketapp;


public class StatusVars {

    public static final int STATUS_RUNNING = 0;
    public static final int STATUS_FINISHED = 1;
    public static final int STATUS_ERROR = 2;
    public static final int STATUS_AVERAGE = 3;
    public static final String AMOUNT_RECEIVED = "amount";
    public static final String DURATION_RECEIVED = "duration";
    public static final String TICKET_ID = "ticketID";
    public static final String ERROR_RECEIVED = "error";
    public static final String IMAGE_RECEIVED = "image";
    public static final String DATE_RECEIVED = "date_received";
    public static final String ROOT_RECEIVED = "root";
    public static final int REDO_OCR = 222;
    public static final String NOT_FOUND = "Not found.";
    public static final String AUTHORITY = "com.ing.software.ticketapp.fileprovider";
    public static final String CORNER1 = "corner1";
    public static final String CORNER2 = "corner2";
    public static final String CORNER3 = "corner3";
    public static final String CORNER4 = "corner4";
    public static final String CORNER5 = "corner5";
    public static final String CORNER6 = "corner6";
    public static final String CORNER7 = "corner7";
    public static final String CORNER8 = "corner8";
}
